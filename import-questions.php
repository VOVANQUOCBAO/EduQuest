...
$requestedCount = max(0, min(100, (int)post('count', 0)));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $aiCount = $requestedCount > 0 ? $requestedCount : max(1, min(100, count($imagePaths) * 6));
    $visionPreview = $imagePaths
        ? gemini_generate_questions_from_images($imagePaths, $type, $aiCount, post('difficulty', 'medium'), $text)
        : [];
    $parsed = attach_image_groups_to_questions($visionPreview ?: parse_questions_text($text, $type), $text);
    ...
    if ($inferredCount === 0) {
        $inferredCount = preg_match_all('/^\s*(?:Cau|Câu|CÃ¢u|CÃƒÂ¢u)\s+\d+\s*[\.:]/miu', $text) ?: 10;
    }
    $fallbackCount = $requestedCount > 0 ? $requestedCount : max(1, min(100, $inferredCount));
  ...
  <form method="post" enctype="multipart/form-data">
    <div class="form-row">
      ...
      <div>
        ...
        <select name="lesson_id" data-lesson-filter="import-lessons" data-empty-label="Mon nay chua co bai hoc tu de da upload">
    ...
    <div class="form-row">
      ...
      <div>
        <label>Độ khó khi dùng AI</label>
        <select name="difficulty"><option value="easy">Nhận biết</option><option value="medium" selected>Thông hiểu</option><option value="hard">Vận dụng</option></select>
      </div>
    </div>
    <div class="form-row">
      <div><label>Số câu khi dùng AI</label><input type="number" name="count" min="1" max="50" value="5"></div>
      <div><label>File TXT/DOCX/PDF/anh</label><input type="file" name="file" accept=".txt,.docx,.pdf,application/pdf,image/png,image/jpeg,image/webp,image/gif" required></div>
    </div>
  ...
  <?php foreach ($preview as $i => $q): ?>
    ...
    <div class="question-edit-card">
      <div class="question-edit-head">
        ...
        <label class="inline-check"><input type="checkbox" name="q[<?= $i ?>][delete]" value="1"> Xóa câu</label>
      </div>
      ...
      <div class="grid grid-2 question-type-panel" data-panel="mc">
        <div data-option-list data-option-name-template="q[<?= $i ?>][options][__LABEL__]" data-answer-select="select[name='q[<?= $i ?>][answer]']">
          <?php foreach (mc_option_labels($q['options'] ?? [], $q['answer'] ?? 'A') as $o): ?><div class="option-row" data-option-label="<?= e($o) ?>"><label>Đáp án <?= $o ?></label><input name="q[<?= $i ?>][options][<?= $o ?>]" value="<?= e($q['options'][$o] ?? '') ?>"></div><?php endforeach; ?>
        </div>
